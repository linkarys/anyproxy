    ...
    */
    ...
    function routeReq(callback){
        if(userRule.shouldUseLocalResponse(req,reqData)){
            logUtil.printLog(color.blue("==>use local rules"));
    ...
    function dealWithLocalResponse(callback){
        userRule.dealLocalResponse(req,reqData,function(statusCode,resHeader,resBody){
            ...
            if (req.isCombo) {
                var stream = combo(req, userRes);
                stream && stream.pipe(userRes);
                return;
...
function watchRulechange(rule) {

    var filesToWatch = [];

    var currentRule = rule ?
        currentRule = pathUtil.resolve(process.cwd(), rule) :
        pathUtil.resolve(process.cwd(), "rule.js");

    if (fs.existsSync(currentRule)) {
        filesToWatch.push(currentRule);
    } else {
        var logText = color.red("can not find rule file at " + rule);
        rule && logUtil.printLog(logText, logUtil.T_ERR);
    }

    var globalRule = pathUtil.join(util.getAnyProxyHome(), "rule_default.js");

    if (fs.existsSync(globalRule)) {
        filesToWatch.push(globalRule);
    }

    var udpateRule = function() {
        var rule = mergeRule(currentRule, globalRule);
    ...
    filesToWatch.forEach((filePath) => {
        fs.watch(filePath, (event, filename) => {
          ...
          // console.log(filePath, event, filename);
          udpateRule();
        });
    });
}

function mergeRule(currentRule, globalRule) {

    var currentRuleConf = {}, globalRuleConf = {};

    try {
        currentRuleConf = require(currentRule);
    } catch (err) {
        // console.log(err);
    }

    try {
        globalRuleConf = util.requireReload(globalRule);
    } catch (err) {
        // console.log(err);
    }

    Object.assign(currentRuleConf, globalRuleConf);

    return currentRuleConf;
...
module.exports.watchRulechange    = watchRulechange;
